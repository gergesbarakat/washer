        ...
        <script>
            ...
            function toggleFullscreen() {
                if (document.fullscreenElement) {
                    // إذا كان في وضع الشاشة الكاملة، اخرج منه
                    document.exitFullscreen();
                } else {
                    // إذا لم يكن في وضع الشاشة الكاملة، اطلب الدخول له
        ...
        <li class="dropdown ml-3">
            <button type="button" class="dropdown-toggle flex items-center">
                ...
                <div class="p-2 md:block text-left">
                    <h2 class="text-sm font-semibold text-gray-800">{{ Auth::guard('user')->user()->name }}</h2>
                    <p class="text-[13px] font-medium text-gray-400">فندق</p>
                </div>
            </button>
            <div
                class="dropdown-menu shadow-md shadow-black/5 z-30 hidden max-w-xs w-full bg-white rounded-md border border-gray-100">
                <a href="{{ route('user.profile.edit') }}"
                    class="dropdown-item text-gray-600 font-semibold text-[13px] px-4 py-2 hover:bg-gray-50">
                    الملف الشخصي
                </a>
                <a href="{{ route('user.profile.edit') }}"
                    class="dropdown-item text-gray-600 font-semibold text-[13px] px-4 py-2 hover:bg-gray-50">
                ...
                <form method="POST" action="{{ route('user.logout') }}">
                    @csrf
                    <button type="submit"
                        class="dropdown-item text-gray-600 font-semibold text-[13px] px-4 py-2 hover:bg-gray-50 w-full text-right">
                        تسجيل خروج
                    </button>
                </form>
            </div>
